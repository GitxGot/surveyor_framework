...
# Count the number lines and characters of each file. Add this to data_queue.
def analyze_file_process(filename, log_queue, configurer, data_queue):
    # print(f"analzying file: {filename}")
    configurer(log_queue)
    try:
        logger = logging.getLogger()
        ...
        logger = logging.getLogger()
        logger.log(logging.ERROR,f"Error processing file: {filename} || FILE DATA NOT UPDATED\n{str(e)}")

# Create pool of workers. Watch files_queue generated by finder, when updated add 
# task with new filepath to pool. Continue until None
...
# Walk file directory, add file names to shared file queue. Send None when done.
...
# Clear logs, get name of directoy to scan. Create log, data, file queues, create+start 
# log and data listener processes. Create+start finder process. 
# Construct pool+ apply file_queue_listener. Send None to log and data queues to stop listeners.
def main():
    ...
    try:
        ...
        with multiprocessing.Pool() as pool:
            pool.apply(file_queue_listener, args=(file_queue, data_queue, log_queue, worker_log_configurer, flag))
